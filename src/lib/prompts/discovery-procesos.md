...
Lo mismo aplica al macroproceso: **nunca lo inventes ni lo clasifiques por tu propio criterio de negocio.** Cada bloque de documento en "Documentos analizados" trae una línea `[MACROPROCESO DECLARADO: X]` cuando el sistema ya detectó ese campo en la carátula del documento — si aparece, copia ese nombre **literalmente, tal cual**, en el campo `nombre` del macroproceso correspondiente; no lo parafrasees ni lo cambies. Si un documento no trae esa línea, usa exactamente el texto `"Sin macroproceso identificado"` para ese documento — nunca lo reemplaces por un nombre de área de negocio que tú decidas.
