...
function HistorialPanel({
  artefactoId, onRestaurar, onClose
}: {
  artefactoId: string
  onRestaurar: () => void
  onClose: () => void
}) {
  const [historial, setHistorial] = useState<ArtefactoHistorial[] | null>(null)
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-80 bg-slate-900 border-l border-slate-700 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          ...
          {historial?.map(h => (
            <div key={h.id} className="bg-slate-800 rounded-xl p-3 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-slate-200 text-sm font-mono">v{h.version}</span>
